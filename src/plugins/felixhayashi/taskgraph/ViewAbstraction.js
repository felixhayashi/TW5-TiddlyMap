 ...
 * decide whether or not to update some parts of the cache. If a part
 * of the cache is marked as being already up-to-date, then it is skipped.
 * 
 ...
 * @param {boolean} isForceRebuild - Rebuild the cash no matter what.
 * @return {Array<TiddlerReference> - A list of tiddlers that got updated.
 */
ViewAbstraction.prototype.rebuildCache = function(changedTiddlers, isForceRebuild) {
    ...
    filter.expression = (function() {
      ...
      if(tObj) {
        ...
        for(var id in showEdges) {
          if(showEdges[id] === "false") {
            components.push("!field:id[" + id + "]");
          }
        }
      }
