...
var executeUpgrade = function(toVersion, curVersion, upgrade) {
  ...
  // issue debug message
  $tw.tmap.logger("debug",  "Upgrading data structure to " + toVersion);      
  // execute fix
  upgrade();
  // update meta
  utils.setEntry($tw.tmap.ref.sysMeta, "dataStructureState", toVersion);
...
fixer.fix = function() {
  ...
  executeUpgrade("0.7.0", meta.dataStructureState, function() {
    ...
    var filter = $tw.tmap.selector.allViews;
  ...
  executeUpgrade("0.7.32", meta.dataStructureState, function() {
    ...
    var liveView = new $tw.tmap.ViewAbstraction("Live View");
  ...
  executeUpgrade("0.9.0", meta.dataStructureState, function() {
    ...
    var confRef = $tw.tmap.ref.visUserConf;
  ...
  executeUpgrade("0.10.3", meta.dataStructureState, function() {
    ...
    var liveTab = $tw.tmap.ref.liveTab;
  ...
  /**
   * Fixes the edge type filter. Before, an empty filter was
  ...
  executeUpgrade("0.11.0", meta.dataStructureState, function() {
    ...
    var views = utils.getMatches($tw.tmap.selector.allViews);
    for(var i = views.length; i--;) {
      var view = new ViewAbstraction(views[i]);
      if(view.getEdgeTypeFilter("raw") === "") {
        var f = $tw.tmap.filter.defaultEdgeTypeFilter;
