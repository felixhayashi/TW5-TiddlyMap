...
(function(){
  ...
  /**
   * Returns all outgoing relations of a tiddler. Which relations are
   * included in the result can be controlled via an optional type-filter
   * and/or an optional to-filter. Per default, all filters act as
   * blacklists.
   * 
   * @param {Tiddler} tiddler - A tiddler object or reference.
   * @param {Hashmap} [options] - An optional options object.
   * @param {Hashmap<TiddlerReference, *>} [options.toFilter.filter]
   ...
   * @param {string} [options.toFilter.type="blacklist"]
   *       Either "blacklist" or "whitelist".
   * @param {Hashmap<string, *>} [options.typeFilter.filter]
   *     A hashmap on which basis it is decided, whether to include
   *     an edge of a given type in the result or not.
   * @param {string} [options.typeFilter.type="blacklist"]
   *       Either "blacklist" or "whitelist".
   * @return {Hashmap<Id, Edge>} All relations of a tiddler represented as edges.
   */
  Adapter.prototype._getRelations = function(tiddler, opts) {
    ...
    var connections = utils.parseFieldData(tRef, this.opt.field.edges, {});
    ...
    for(var conId in connections) {
      ...
      if(isWhitelistToFiltr ? toFiltr[toTRef] : !toFiltr[toTRef]) {
        if(isWhitelistTyFiltr ? tyFiltr[con.type] : !tyFiltr[con.type]) {
          edges[conId] = this.makeEdge(this.getId(tRef), con.to, con.type, conId);
        }
      }
    }
  ...
  /**
   * Returns all outgoing *links* of a tiddler. Which links are
   * included in the result can be controlled via an optional to-filter.
   * Per default, the filter acts as blacklists.
   * 
   * @param {Tiddler} tiddler - A tiddler object or reference.
   ...
   * @return {Hashmap<Id, Edge>} All relations of a tiddler represented as edges.
   */
  Adapter.prototype._getLinks = function(tiddler, opts) {
    ...
    var edges = utils.getDataMap();
    var type = new EdgeType("tmap:link");
   ...
   * virtual edges (links) and user created relations.
   * 
   * @param {Hashmap<TiddlerReference, *>} [opts.toFilter.filter]
   *     A hashmap on which basis it is decided, whether to include
   *     an edge with a certain to-value in the result or not.
  ...
  Adapter.prototype.getEdges = function(tiddler, opts) {
    ...
    // get relations
    var relations = this._getRelations(tiddler, opts);
    $tw.utils.extend(edges, relations);
    
    // get links
    if(isWhitelistTyFiltr ? tyFiltr["tmap:link"] : !tyFiltr["tmap:link"]) {
      $tw.utils.extend(edges, this._getLinks(tiddler, opts));
    }
