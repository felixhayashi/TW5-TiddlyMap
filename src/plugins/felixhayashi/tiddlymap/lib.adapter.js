...
Adapter.prototype.getNeighbours = function(matches, opts) {
  ...
  var isTraceAllEdges = (!opts.direction || opts.direction === "both");
  var isTraceIncomingEdges = (isTraceAllEdges || opts.direction === "to");
  var isTraceOutgoingEdges = (isTraceAllEdges || opts.direction === "from");
  
  // adjacency receives whitelists through opts
  var adjList = (isTraceIncomingEdges
                 ? this.getAdjacencyList("to", opts)
                 : {});
  ...
  for(var step = 0; step < maxSteps && matches.length; step++) {
    ...
    for(var i = matches.length; i--;) {
      ...
      for(var id in outgoing) addAsNeighbour(outgoing[id], "to");
