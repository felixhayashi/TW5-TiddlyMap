...
MapWidget.prototype.checkForFreshInstall = function() {
  ...
  this.dialogManager.open(name, args, function(isConfirmed, outTObj) {
    ...
    var view = this.opt.misc.defaultViewLabel;
...
MapWidget.prototype.refresh = function(changedTiddlers) {
  ...
  // = we are in a new view
    ...
    if(viewModifications.length && !this.ignoreNextViewModification) {
      ...
      reinitNetwork = !!viewModifications["config"];
...
MapWidget.prototype.toggleEnlargedMode = function(type) {
  ...
  if(!enlargedMode || enlargedMode !== type) {
  // either not active or we switched from one mode to another
    
    if(!this.isContainedInSidebar && type === "halfscreen") {
      // not allowed
      return;
    }
...
MapWidget.prototype.handleEditNode = function(node) {
  ...
  var args = {
    ...
    "tidLabelField": "global." + this.opt.field.nodeLabel,
    "tidIconField": "global." + this.opt.field.nodeIcon,
    dialog: {
      preselects: {
  ...
  addToPreselects("global", tObj.fields, [
    this.opt.field.nodeLabel,
    this.opt.field.nodeIcon,
    "tmap.fa-icon",
    "tmap.open-view"
  ]);
...
MapWidget.prototype.destruct = function() {
  ...
  this.graphDomNode.removeEventListener('keyup', this.graphKeyupHandler, true);
...
MapWidget.prototype.getView = function(noCache) {
  ...
  if(!view.exists()) {
    this.logger("debug", "Warning: View \"" + view.getLabel() + "\" doesn't exist. Default is used instead.");
